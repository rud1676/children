...
export async function GET(request) {
  try {
    ...
    const studentsQuery = `
      ...
      FROM users u
      LEFT JOIN praises p ON u.id = p.to_user_id AND p.is_deleted = FALSE
      WHERE u.role = 'student'
      ORDER BY u.id, p.created_at DESC
    `;

    const students = db.prepare(studentsQuery).all();
    ...
    if (isSpecialUser) {
      ...
      if (currentSelectedStudents.length === 0) {
        ...
        currentSelectedStudents = shuffledStudents.slice(0, 8).map((s) => s.id);
      } else {
